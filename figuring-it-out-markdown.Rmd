...
Trains are a timeless mode of transportation that has revolutionized the world. As we aim to improve their reliability and accessibility, it is crucial to understand their performance records and allow riders to stay informed on travel updates, train reliability, and enable them to plan their trips accordingly. Hence, we have dedicated this brief to developing a two-part prediction model to forecast train delays, and connect this model to a concept mobile app that would connect multiple-transit groups and inform riders on expected delays. The purpose of the app is to incorporate all major train companies in the New York City area (MTA, Amtrack, NJ Transit, etc) by centralizing the ability to purchase train tickets in one app, be notified of delays or cancellations, and see performance metrics on train quality for adaptive planning. 
...
ggplot(may_w_stations,mapping=aes(x=delayed))+
  ...
  labs(title="Delayed Train Trip Count for Northeast Corridor Line, May, 2020",
       y="Number of trips",
       x = "Delayed")+
